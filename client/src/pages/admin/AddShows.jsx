  ...
  return nowPlayingMovies.length > 0 ? (
    <div>
        ...
        <div className='group flex flex-wrap gap-4 mt-4 w-max'>
          {
            nowPlayingMovies.map((movie) => (
              <div onClick={() => setSelectedMovie(movie.id)} key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`}>
                ...
                <p className=' font-medium truncate text-xl'>{movie.title}</p>
      ...
      <div className='mt-8 text-sm'>
        <label htmlFor="" className='block text-lg font-medium mb-2'>Show Price</label>
      ...
      <div className='mt-6 text-sm'>
        <label htmlFor="" className='block text-lg font-medium mb-2' onClick={handleDateTimeAdd}>
      ...
      <button onClick={handleSubmit} disabled={addingShow} className=' text-lg bg-primary text-white px-8 py-2 mt-6 rounded hover:bg-primary/90 transition-all cursor-pointer'>Add Show</button>
    </div>
  ) : <Loading />
}
